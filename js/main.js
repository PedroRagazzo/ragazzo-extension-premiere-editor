(function () {
  ...
  // Curva medida (barras) x curva de ganho aplicada (linha) de cada clipe,
  // desenhada logo após a medição — dá pra ver visualmente as "inúmeras
  // keys com valores diferentes" que o nivelamento cria, em vez de um
  // único número escondido no Effect Controls do Premiere.
  function drawLevelGraphs(plans) {
    var container = document.getElementById("level-graph-grid");
    container.innerHTML = "";
    ...
    var colBar = themeColor("--border", "#444");
    var colLine = themeColor("--accent", "#ff7a26");
    var colText = themeColor("--text-dim", "#999");

    var minDb = -60, maxDb = 15;
    function yFor(db, h) {
    ...
    plans.forEach(function (p) {
      if (!p.windows || p.windows.length === 0) return;

      var wrap = document.createElement("div");
      wrap.className = "level-graph-card";

      var label = document.createElement("span");
      label.className = "level-graph-label";
      label.textContent = "Clipe " + (p.index + 1) + " — " + p.keys.length + " keyframe(s)";
      wrap.appendChild(label);

      var canvas = document.createElement("canvas");
      canvas.width = 460;
      canvas.height = 90;
      canvas.className = "level-graph-canvas";
      ...
      var n = p.windows.length;
      var slot = w / n;

      // barras: volume médio medido (referência — "antes")
      ctx.fillStyle = colBar;
      p.windows.forEach(function (win, i) {
        var y = yFor(win.meanVolumeDb, h);
        ctx.fillRect(i * slot + 1, y, Math.max(1, slot - 2), h - y);
      });

      // linha: ganho resultante — é isso que vira keyframe ("depois")
      ctx.strokeStyle = colLine;
      ctx.lineWidth = 2;
      ctx.beginPath();
      p.windows.forEach(function (win, i) {
        var x = i * slot + slot / 2;
        var y = yFor(win.gainDb, h);
      ...
      ctx.fillText("volume medido (barras) · ganho aplicado (linha)", 2, h - 3);
    });
  }

  document.getElementById("apply-level").addEventListener("click", function () {
    var target = parseFloat(document.getElementById("target-db").value);
    var windowSec = parseFloat(document.getElementById("level-window").value);
    ...
    CepBridge.evalScript("getSelectedAudioSourcePaths()", function (result) {
      ...
      Loudness.planLeveling(
        items,
        target,
        windowSec,
        function (clipDone, clipTotal, winDone, winTotal) {
          setLoading("Medindo clipe " + clipDone + "/" + clipTotal + " (janela " + winDone + "/" + winTotal + ")...");
        },
        function (plans) {
          drawLevelGraphs(plans);

          var totalKeys = 0;
          var clampedWindows = 0;
          plans.forEach(function (p) {
            totalKeys += p.keys.length;
            p.windows.forEach(function (w) { if (w.clamped) clampedWindows++; });
          });

          if (totalKeys === 0) {
            setMessage("Não consegui medir volume em nenhum trecho dos clipes selecionados.", "err");
            return;
          }

          setLoading("Aplicando " + totalKeys + " keyframe(s)...");
          var payload = plans.map(function (p) { return { index: p.index, keys: p.keys }; });

          CepBridge.evalScript("applyLevelKeyframesToSelection(" + JSON.stringify(JSON.stringify(payload)) + ")", function (result2) {
            if (typeof result2 === "string" && result2.indexOf("ok:") === 0 && clampedWindows > 0) {
              // O parâmetro Nível do Premiere trava em +15dB (ver LEVEL_MAX_DB em
              // loudness.js) — quando um trecho precisaria de mais ganho que isso,
              // essa janela específica fica presa em 15dB. Avisa em vez de deixar
              // parecer que o alvo foi alcançado em todo o clipe.
              setMessage(
                "Aplicado a " + result2.slice(3) + ". Atenção: " + clampedWindows +
                  " trecho(s) precisariam de mais ganho do que o parâmetro \"Nível\" do Premiere permite (máx. +15dB) pra alcançar " +
                  target + " dB — apliquei o máximo possível nesses trechos.",
                "ok"
              );
            } else {
              setStatus(result2);
